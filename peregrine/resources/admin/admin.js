...
(function ($) {
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        ...
                        case 8:
                            ...
                            wp.ajax.post('peregrine_front_set_predesign', {
                                index: selectedFrontPage,
                                nonce: peregrine_builder_status.peregrine_front_set_predesign_nonce
                            });
                            if (peregrine_builder_status.status === "not-installed") {
                                toggleProcessing(true);
                                installBuilder(callback);
                            }

                            if (peregrine_builder_status.status === "installed") {
                                toggleProcessing(true);
    ...
    var $notice_container = $('.peregrine-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('peregrine_disable_big_notice', {
            nonce: peregrine_builder_status.peregrine_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.peregrine-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.peregrine-admin-big-notice').addClass('processing');
            $('.peregrine-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.peregrine-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(peregrine_builder_status.messages.installing);
        $.get(peregrine_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.peregrine-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(peregrine_builder_status.messages.activating);

        wp.ajax.post('peregrine_activate_plugin', {
            slug: peregrine_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: peregrine_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.peregrine-admin-big-notice');
            if (callback) {
                callback();
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = peregrine_builder_status.view_demos_url;
        });
    });
    ...
    var peregrineInstallPluginSuccess = function peregrineInstallPluginSuccess(response) {
    ...
    var peregrineInstallPlugin = function peregrineInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: peregrineInstallPluginSuccess
        });
    };

    var peregrineActivatePlugin = function peregrineActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", peregrineInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", peregrineActivatePlugin);
